...
export function emptyReceipt(rawText = ""): ReceiptData {
  return {
    supplier: "",
    date: "",
    invoiceNumber: "",
...
export async function readReceiptFromImageFile(file: File) {
  ...
  if (file.size <= 0) {
    throw new Error("Geen geldig bonbestand ontvangen.");
  }

  if (file.size > 8 * 1024 * 1024) {
    throw new Error("Bestand is te groot. Upload maximaal 8 MB.");
  }
...
- Gebruik alleen bedragen die zichtbaar op de bon staan of logisch volgen uit totaal en btw.`;
  ...
  if (!response.ok) {
    throw new Error(data?.error?.message || `AI OCR mislukt met status ${response.status}.`);
  }
  ...
  return normalizeReceipt(extractJson(outputText), outputText);
}
