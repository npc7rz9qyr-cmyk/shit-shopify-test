...
function effectiveVatRate(exVatCents: bigint, vatCents: bigint) {
  if (exVatCents <= 0n || vatCents <= 0n) return 0;
  return Number((vatCents * 10000n) / exVatCents) / 100;
}

function addToBox(box: { amount: bigint; vat: bigint }, amount: bigint, vat: bigint) {
  box.amount += amount;
  box.vat += vat;
}
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const vatBoxes = {
    high: { amount: 0n, vat: 0n },
    low: { amount: 0n, vat: 0n },
    zero: { amount: 0n, vat: 0n },
    other: { amount: 0n, vat: 0n },
  };
  ...
  return (
    <s-page heading="Boekhouding">
      ...
      <s-section heading={`BTW-aangifte overnemen ${data.periodLabel}`}>
        <s-banner tone="warning">Controleer dit altijd vóór indienen. De app detecteert nu hoog/laag/0/overig via effectief btw-tarief per order. EU/OSS/export/verlegd blijven handmatig totdat die gegevens apart worden vastgelegd.</s-banner>
        <div style={{ overflowX: "auto", marginTop: "1rem" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "58rem" }}>
            <thead><tr><th style={{ textAlign: "left", borderBottom: "1px solid #dfe3e8", padding: "0.6rem" }}>Rubriek</th><th style={{ textAlign: "left", borderBottom: "1px solid #dfe3e8", padding: "0.6rem" }}>Omschrijving</th><th style={{ textAlign: "right", borderBottom: "1px solid #dfe3e8", padding: "0.6rem" }}>Omzet / waarde excl. btw</th><th style={{ textAlign: "right", borderBottom: "1px solid #dfe3e8", padding: "0.6rem" }}>Btw-bedrag</th><th style={{ textAlign: "left", borderBottom: "1px solid #dfe3e8", padding: "0.6rem" }}>Opmerking</th></tr></thead>
            <tbody>{data.declarationRows.map((row) => <tr key={row.code}><td style={{ borderBottom: "1px solid #f1f2f4", padding: "0.6rem", fontWeight: 700 }}>{row.code}</td><td style={{ borderBottom: "1px solid #f1f2f4", padding: "0.6rem" }}>{row.label}</td><td style={{ borderBottom: "1px solid #f1f2f4", padding: "0.6rem", textAlign: "right", whiteSpace: "nowrap" }}>{formatEuros(BigInt(row.amountCents))}</td><td style={{ borderBottom: "1px solid #f1f2f4", padding: "0.6rem", textAlign: "right", whiteSpace: "nowrap" }}>{formatEuros(BigInt(row.vatCents))}</td><td style={{ borderBottom: "1px solid #f1f2f4", padding: "0.6rem" }}>{row.note}</td></tr>)}</tbody>
          </table>
        </div>
      </s-section>
